...
OUTPUT_DIR = BASE_DIR / "plots_accuracy_and_detection_natural_yaxis"
SEED = 42
LIVE_ROUNDS = 100
...
# Accuracy y-axis.
# None means Matplotlib will automatically choose the y-axis range based on the data.
TEST_ACC_YLIM = None
...
def plot_test_accuracy_for_attack(attack_type: str) -> None:
    ...
    fig, axes = plt.subplots(
        ...
        figsize=(24, 12),
        sharex=True,
        sharey=False,  # keep False so each subplot gets its own natural y-axis range
    )
    ...
    for row_idx, attacker_pct in enumerate(ATTACK_PCTS):
        for col_idx, alpha in enumerate(ALPHAS):
            ...
            # Natural y-axis scaling for test accuracy.
            # If you later want fixed 0--100 scaling again, set TEST_ACC_YLIM = (0, 100)
            # and uncomment the fixed y-ticks below.
            if TEST_ACC_YLIM is not None:
    ...
    save_figure(fig, f"{attack_type}_test_accuracy_line_natural_yaxis")
    plt.close(fig)
...
def plot_detection_dr_fdr_bars_for_attack(attack_type: str) -> None:
    ...
    fig, axes = plt.subplots(nrows=2, ncols=1, figsize=(18, 11), sharex=True)
    ax_dr, ax_fdr = axes
    ...
    ax_dr.legend(fontsize=LEGEND_SIZE, frameon=True, loc="lower right")
    fig.tight_layout()
