# fl_project/stage_2_live_detection_signflip.py
...
def main():
    """Runs the live detection phase (Stage 2) for SIGN FLIPPING attack."""
    print(f"--- 🟢 STAGE 2: Live Detection (Sign Flipping Attack, Rounds {cfg.R0_PRETRAIN_ROUNDS + 1} to {cfg.T_TOTAL_ROUNDS}) ---")
    ...
    # --- 2. Setup Logging ---
    log_subdir = f"{cfg.PERCENTAGE_ATTACKERS}p_alpha_{cfg.ATTACK_PROBABILITY}_signflip_{cfg.DEFENSE_METHOD}"
    ...
    # NEW: pre-flip attacker snapshots
    ...
    for round_num in range(cfg.R0_PRETRAIN_ROUNDS + 1, cfg.T_TOTAL_ROUNDS + 1):
        ...
        for client_id in tqdm(selected_clients, desc="Local Client Training"):
            ...
            # 🚨 SIGN FLIP ATTACK: after training, invert weights/biases
            if is_actual_attacker:
                preflip_h5 = os.path.join(preflip_dir, f"Round_{round_num}_Client_{client_id}_PREATTACK.h5")
                with h5py.File(preflip_h5, "w") as hf_pre:
                    for name, param in local_model.state_dict().items():
                        hf_pre.create_dataset(name, data=param.detach().cpu().numpy())

                flipped_weights = {k: -1 * v for k, v in local_model.state_dict().items()}
                local_model.load_state_dict(flipped_weights)
            ...
            hdf5_path = os.path.join(local_weights_dir, f"Round_{round_num}_Client_{client_id}_{'Attacker' if is_actual_attacker else 'Benign'}.h5")
            with h5py.File(hdf5_path, "w") as hf:
                for name, param in local_model.state_dict().items():
                    hf.create_dataset(name, data=param.cpu().numpy())

        # c. Server-side Detection Phase
        ...
        for client_id, local_weights in tqdm(client_updates.items(), desc="Server-Side Detection"):
            ...
            # Calculate alignment feature for this specific client update
            #alignment_feature = F.cosine_similarity(conv2_update.view(32, -1), benign_ref_dirs.view(32, -1), dim=1)
        ...
        # d. Secure Aggregation (thresholding or credit scoring, same as before)
        aggregation_weights, TP, FP, TN, FN = {}, 0, 0, 0, 0

        # Decide aggregation weights based on the chosen defense method
        if cfg.DEFENSE_METHOD == 'thresholding':
            ...
            # Calculate the denominator of the credit score formula: Σ nʲ (Aʲ)⁻ᴸ
        ...
        print(f" Attackers: {actual_attackers_in_round if actual_attackers_in_round else 'None'}")
        ...
        print(f"\n🎯 Round {round_num}: Test Accuracy: {test_acc:.2f}%| Precsion: {prec:.2f}% | Recall: {rec:.2f}% | F1 Score: {f1:.2f}%")
